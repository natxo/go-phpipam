...
is_gateway is a string on the receiver side, so phpipam expects a '0' or '1'
as a string ...
*/
func (c *Client) CreateAddressFirstFree(subnetID, hostname, description, owner string, isgw bool) (AddressFirstFree, error) {
	var addressFirstFreeData AddressFirstFree
	var newip Addressadd
	newip.Hostname = hostname
	...
	req, _ := http.NewRequest("POST", c.ServerURL+"/api/"+c.Application+"/addresses/first_free/"+subnetID+"/", bytes.NewBuffer(data))
	body, err := c.Do(req)
	if err != nil {
		return addressFirstFreeData, err
	}
